...
__export(index_exports, {
  ...
  useDiceRoll: () => useDiceRoll,
  useWheelSpin: () => useWheelSpin
});
...
// src/react/useWheelSpin.ts
var import_react = require("react");
function useWheelSpin(options, onComplete) {
  const [isSpinning, setIsSpinning] = (0, import_react.useState)(false);
  const [currentAngle, setCurrentAngle] = (0, import_react.useState)(0);
  const [winner, setWinner] = (0, import_react.useState)(null);
  const animFrameRef = (0, import_react.useRef)(null);
  const physicsRef = (0, import_react.useRef)(new WheelPhysics(options));
  const spin = (0, import_react.useCallback)(
    (items) => {
...
// src/react/useCoinFlip.ts
var import_react2 = require("react");
function useCoinFlip(language = "de") {
  const [isFlipping, setIsFlipping] = (0, import_react2.useState)(false);
  const [result, setResult] = (0, import_react2.useState)(null);
  const flip = (0, import_react2.useCallback)(() => {
    setIsFlipping(true);
    setResult(null);
    setTimeout(() => {
...
// src/react/useDiceRoll.ts
var import_react3 = require("react");
function useDiceRoll() {
  const [isRolling, setIsRolling] = (0, import_react3.useState)(false);
  const [result, setResult] = (0, import_react3.useState)(null);
  const roll = (0, import_react3.useCallback)((notation) => {
    setIsRolling(true);
    setResult(null);
    setTimeout(() => {
      try {
